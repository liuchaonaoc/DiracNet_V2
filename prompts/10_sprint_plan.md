...
> 总目标：把 V2 从空目录推到 Stage 2 通过、Phase 1 NIST 全量 RMS < 50 meV，
...
## Sprint 3 — Stage 2 + KAN 升级 + Phase 1 全量（3-4 天）

| 步骤 | 关联文件 |
|------|---------|
| 实现 `models/level_residual_head.py` | `06_two_stage_training.md` §3.3 |
| 扩展 `training/two_stage_trainer.py` 加入 Stage 2 分支 + rollback | `06_two_stage_training.md` §3.4 |
...
```bash
# A. 用 60 行 extended hydrogenic 跑完整 Stage 1 + Stage 2
...
# 期望:
#   aligned RMS < 500 meV (chemical accuracy ~1 eV)
#   median |error| < 100 meV
#   |cos| min > 0.95 across hydrogenic test subset
#   |Δ_res| max < 100 meV (一些重原子可能稍大)
```

最终 deliverable：

- `results/v2_phase2/SUMMARY.md` 完整对比 V1 R9 / B16 / B9-LOO 与 V2 Stage 1 / Stage 2
- `figures/` 完整可视化
...
| M4: Sprint 3 done | day 11 | Stage 2 RMS < 50 meV on 60 行 |
| M5: Sprint 4 done | day 18 | Phase 2 RMS < 500 meV |
...
| Sprint 3: Stage 2 rollback 频繁 | NIST 数据噪声 / Δ_max 太小 | `Δ_max: 50 → 100 meV`，`w_nist_max: 1.0 → 0.3` |
